...
// Special instructions testbench: mfhi and mflo
...
module special_tb;
    ...
    initial begin
        $dumpfile("special.vcd");
        $dumpvars(0, special_tb);
        #300;
        $display("Special mfhi/mflo test complete");
        $display("R5 (from HI) = 0x%h", R5);
        $display("R1 (from LO) = 0x%h", R1);
        $finish;
    end
